...
export default function Claim({ auth, workbook, relation, agent, policy, causeOfLoss }: any) {
    ...
    const createSelectOptions = (data: any[], labelKey: string, valueKey: string) => {
    ...
    const causeOfLossOptions = createSelectOptions(causeOfLoss, "CAUSE_OF_LOSS_NAME", "CAUSE_OF_LOSS_ID");
    ...
    const coveragesOptions = coverages[0]?.map((el: any) => {
        return {
    ...
    const removeRowPolis = (index: number) => {
        setData({
            ...data,
            potentialInsurancePolicy: data.potentialInsurancePolicy.filter((_: any, i: number) => i !== index)
        })
    }
    ...
    const removeRowClaimValue = (index: number) => {
        setData({
            ...data,
            claimValueSubmitted: data.claimValueSubmitted.filter((_: any, i: number) => i !== index)
        })
    }
    ...
    const removeRow = (e: FormEvent) => {
        ...
        setData({
            ...data,
            potentialInsurancePolicy: data.potentialInsurancePolicy.filter((_: any, i: number) => i !== data.potentialInsurancePolicy.length - 1),
    ...
    const addCoverage = (e: FormEvent, index: any) => {
        ...
        setData({
            ...data,
            potentialInsurancePolicy: data.potentialInsurancePolicy.map((el: any, i: number) => {
                if (i === index) {
                    return {
                        ...el,
                        coverage: [
    ...
    const removeInterestInsured = (e: FormEvent, policyIndex: number, coverageIndex: number) => {
        e.preventDefault();
        setData({
            ...data,
            potentialInsurancePolicy: data.potentialInsurancePolicy.map((policy: any, i: number) => {
                if (i === policyIndex) {
                    return {
                        ...policy,
                        coverage: policy.coverage.map((coverage: any, j: number) => {
                            if (j === coverageIndex) {
                                return {
                                    ...coverage,
                                    interestInsured: coverage.interestInsured.filter((_: any, k: number) => k !== coverage.interestInsured.length - 1)
                                };
                            }
                            return coverage;
                        })
    ...
    const removeCoverage = (e: any, policyIndex: number,) => {
        e.preventDefault();
        const coverageIndex = data.potentialInsurancePolicy[policyIndex].coverage.length - 1;
        setData({
            ...data,
            potentialInsurancePolicy: data.potentialInsurancePolicy.map((policy: any, i: number) => {
                if (i === policyIndex) {
                    return {
                        ...policy,
                        coverage: policy.coverage.filter((_: any, j: number) => j !== coverageIndex)
                    };
                }
                return policy;
            })
    ...
    function Stepper() {
        ...
        function MilestoneItem({ milestone, completedMilestones, handleCheckboxChange, handleFileChange }: any) {
            return (
                <div className="rounded-md p-2 mt-4">
                    <fieldset className="border border-gray-400 p-3 rounded-md flex-1">
                        <legend className="text-sm font-semibold px-2 text-gray-600">{milestone?.MILESTONE_NAME}</legend>
                        <div className="flex gap-6">
                            <div className="flex-1">
                                <label htmlFor={`input-${milestone?.MILESTONE_ID}`} className="text-sm text-gray-800">
                                    Note
                                </label>
                                <TextInput
                            ...
                            <div className="mt-3 flex items-center">
                                <input
                                    ...
                                    checked={completedMilestones[milestone?.MILESTONE_ID] || false}
                                    onChange={() => handleCheckboxChange(milestone?.MILESTONE_ID)}
                                />
                                <label htmlFor={`checkbox-${milestone?.MILESTONE_ID}`} className="text-sm text-gray-800">
                                    Completed
                                </label>
                            </div>
                            <div className="mt-2">
                                <label htmlFor={`file-${milestone?.MILESTONE_ID}`} className="text-sm text-gray-800">
                                    Upload Document
                                </label>
                                <input
                                    type="file"
                                ...
                                hover:file:bg-blue-100"
                                    ...
                                    onChange={(e) => {
                                        if (e.target.files && e.target.files.length > 0) {
                                            handleFileChange(milestone?.MILESTONE_ID, e.target.files[0]);
                                        }
                                    }}
                                />
                            </div>
                        ...
                        {milestone?.children && milestone?.children?.length > 0 && (
                            <>
                                <div className="mt-4 ml-4">
                                    {milestone.children.map((child: any) => (
                                        <MilestoneItem
        ...
        return (
            <div className="w-full mx-auto p-4">
              ...
              ${index === currentStep ? "bg-blue-600 border-4 border-blue-300 shadow-lg" :
                                    ...
                                    {index < milestones?.length - 1 && (
                                        <div className={`flex-1 h-1 w-full ${index < currentStep ? "bg-green-500" : "bg-gray-300"}`} />
                                    )}

                                    <p className="h-10 text-center  text-xs font-medium text-gray-700">{milestone?.MILESTONE_NAME}</p>
                    ...
                    <fieldset className="border border-gray-300 p-6 rounded-md">
                        <legend className=" text-gray-700">
                            <div className="p-2 rounded-md">
                                <p className="text-lg font-semibold">{milestones[currentStep].MILESTONE_NAME}</p>
                                <p className="ml-1 italic text-xs text-red-500">{milestones[currentStep].MILESTONE_DURATION_DESCRIPTION}</p>
                            </div>
                        </legend>

                        <div className="">
                            {milestones[currentStep].children.map((child: any, index: number) => (
                                <MilestoneItem
                                    key={index}
                                    milestone={child}
                                    completedMilestones={completedMilestones}
    ...
    const valueRevChange = (e: any, data: any) => {
        e.preventDefault()
        console.log(data.CLAIM_COVERAGE_ID, 'data');
    ...
    const coverageBySelect = coveragePolicy && coveragePolicy[0]?.filter((el: any) => {
        return el.POLICY_COVERAGE_ID === data?.potentialInsurancePolicy[0]?.coverage?.[0]?.coverage
    })

    const coverageInsured = coverageBySelect?.map((el: any) => { return el })
    ...
    const coverageInsuredDetailOptions = coverageInsuredDetail && coverageInsuredDetail[0]?.map((el: any) => {
        return {
            label: el.interest_insured?.
                                        ...
                                        <div className="rounded-lg mt-2 ">
                                            ...
                                            <div className="bg-slate-200 font-medium rounded-md flex gap-2 justify-between text-center p-2 h-24 text-md mt-3">
                                                ...
                                                <div className="w-full rounded-lg h-10 mt-2">
                                                    <div className="text-md">{el?.policy[0]?.POLICY_THE_INSURED}</div>
                                                    <div className="text-xs text-gray-400 mt-2"> {el?.NOTE}</div>
                                                </div>
                                                <div className="w-full rounded-lg h-10 mt-2 ">
                                                ...
                                                <div className="w-full rounded-lg h-10 mt-2 flex flex-col ">
                                                    ...
                                                    <div className="text-md">{el?.cause_of_loss[0]?.CAUSE_OF_LOSS_NAME}</div>
                                                    <div className="text-xs text-gray-400 mt-2"> {el?.NOTE_CAUSE_OF_LOSS}</div>
                                                </div>
                                            </div>
                                                                ...
                                                                <fieldset className="">
                                                                    ...
                                                                    <div className="flex gap-2">
                                                                        <div className="bg-grey-50 border-gray-400 border rounded-md w-full p-4">
                                                                            {el.coverage?.POLICY_COVERAGE_NAME}
                                                                    ...
                                                                    <fieldset className="mt-2 p-4 justify-end border border-gray-400 rounded-md">
                                                                        ...
                                                                        {
                                                                            el?.insured?.reduce((acc: any, curr: any) => {
                                                                                const existing = acc.find((item: any) => item.POLICY_COVERAGE_DETAIL_ID === curr.claim_coverage.POLICY_COVERAGE_DETAIL_ID);
                                                                                if (existing) {
                                                                                    existing.values.push(curr);
                                                                                } else {
                                                                                    acc.push({
                                                                                        POLICY_COVERAGE_DETAIL_ID: curr.claim_coverage.POLICY_COVERAGE_DETAIL_ID,
                                                                                        INTEREST_INSURED_NAME: curr.claim_coverage.interest_insured.INTEREST_INSURED_NAME,
                                                                                        values: [curr]
                                                                                    });
                                                                                }
                                                                                return acc;
                                                                            }, []).map((group: any) => {
                                                                                const currency = group.values[0]?.claim_coverage?.currency?.CURRENCY_SYMBOL
                                                                                return (
                                                                                    <>
                                                                                        <div key={group.POLICY_COVERAGE_DETAIL_ID} className="mt-2 flex gap-2 justify-between ">
                                                                                                ...
                                                                                                {group.values.length > 0 && (
                                                                                                    <>
                                                                                                        <div className=" bg-slate-200 border-gray-300 border rounded-md w-40 p-2 mx-2 text-center">
                                                                                                            <div className="text-md ">{group.values[group.values.length - 1].VALUE_SUBMIT_COVERAGE}</div>
                                                                                                            <div className="text-xs italic text-gray-400"> Note : {group.values[group.values.length - 1].NOTE_VALUE_SUBMIT_COVERAGE}</div>
                                                                                                        </div>
                                                                                                        <div className="flex-col space-y-3 mt-1">
                                                                                                            <div className="text-gray-500 text-sm flex flex-col gap-2 italic hover:text-red-400 cursor-pointer"
                                                                                                        ...
                                                                                                        <div className="text-center border border-gray-300 bg-slate-200 w-full  flex items-center rounded-md">
                                                                                                            ...
                                                                                                            <div className="text-2xl text-green-400 font-serif w-full  ">{group.values[group.values.length - 1].VALUE_SUBMIT_COVERAGE}</div>
                                                                                                        </div>
                                            ...
                                            <div className="flex gap-2 justify-end mb-2 p-">
                                                <div className="mt-2 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                    onClick={(e) => addRow(e)}>
                                                    <PlusCircleIcon className="w-5 h-5" /> action 1
                                                </div>
                                                <div className="mt-2 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                    onClick={(e) => removeRow(e)}>
                                                    <MinusCircleIcon className="w-5 h-5" /> action 2
                                                </div>
                                            </div>
                    ...
                    <>
                        <div className="flex gap-4 justify-between">
                            <div className="mt-2 w-full">
                                ...
                                <Select
                                    ...
                                    value={
                                        relationOptions.find((el: { value: any }) => el.value === data.client) || null
                                    }
                                    onChange={(e: any) => {
                                        setData({
                                            ...data,
                            ...
                            <div className="mt-2 w-full">
                                ...
                                <Select
                                    ...
                                    placeholder="Select Client Name"
                                    value={
                                        workbookOptions.find((el: { value: any }) => el.value === data.workbook) || null
                                    }
                                    onChange={(e: any) => {
                                        setData({
                                            ...data,
                        ...
                        <div className=" border-2 border-gray-400 rounded-md bg-gray-100 lg:flex lg:gap-4 lg:justify-between mt-4 p-4">
                            <div className="w-full">
                                ...
                                {
                                    data?.potentialInsurancePolicy?.map((row: any, index: number) => (
                                        <>
                                                    ...
                                                    <Select
                                                        ...
                                                        onChange={(e: any) => {
                                                            ...
                                                            updatedPolicies[index] = {
                                                                potentialInsurancePolicy: e ? e.value : null,
                                                                note: ""
                                                            };
                                                            // Update state dengan array yang telah diperbarui
                                                            setData({
                                                                ...data,
                                                                potentialInsurancePolicy: updatedPolicies
                                                            });
                                                        }}
                                                        value={
                                                            // Tentukan nilai value yang benar, atau null jika tidak ada yang dipilih
                                                            data.potentialInsurancePolicy[index]
                                                                ? policyOptionsFilter.find((el: { value: any }) => el.value === data.potentialInsurancePolicy[index]?.potentialInsurancePolicy) || null
                                                                : null
                                                        }
                                                    />
                                                    {/* end select policy */}
                                                    ...
                                                    <TextArea
                                                        ...
                                                        onChange={
                                                            (e: any) => {
                                                                const updatedPolicies = [...data.potentialInsurancePolicy];
                                                                updatedPolicies[index] = {
                                                                    potentialInsurancePolicy: data.potentialInsurancePolicy[index]?.potentialInsurancePolicy,
                                                                    note: e.target.value
                                                                };
                                                                setData({
                                                                    ...data,
                                                        ...
                                                        value={data.potentialInsurancePolicy[index]?.note}
                                                    />
                                                    <span className="ml-2 text-sm italic text-gray-400"># add note if needed</span>
                                                    {/* end input note */}
                                                ...
                                                <div key={index} className="mt-2 w-full">
                                                    ...
                                                    <Select
                                                        ...
                                                        onChange={(e: any) => {
                                                            ...
                                                            updatedPolicies[index] = {
                                                                ...updatedPolicies[index],
                                                                causeOfLoss: e ? [{ causeOfLoss: e.value, note: "" }] : []
                                                            };
                                                            setData({
                                                                ...data,
                                                                potentialInsurancePolicy: updatedPolicies
                                                        ...
                                                        value={
                                                            data.potentialInsurancePolicy[index]?.causeOfLoss?.length > 0
                                                                ? causeOfLossOptions.find((el: { value: any }) => el.value === data.potentialInsurancePolicy[index]?.causeOfLoss[0]?.causeOfLoss) || null
                                                                : null
                                                        }
                                                    />
                                                    {/* end cause of loss */}
                                                    ...
                                                    <TextArea
                                                        ...
                                                        onChange={(e: any) => {
                                                            const updatedPolicies = [...data.potentialInsurancePolicy];
                                                            if (updatedPolicies[index]?.causeOfLoss?.length > 0) {
                                                                updatedPolicies[index].causeOfLoss[0].note = e.target.value;
                                                            }
                                                            setData({
                                                                ...data,
                                                                potentialInsurancePolicy: updatedPolicies
                                                        ...
                                                        value={data.potentialInsurancePolicy[index]?.causeOfLoss?.[0]?.note || ""}
                                                    />
                                                    <span className="ml-2 text-sm italic text-gray-400"># add note if needed</span>
                                            ...
                                            <div className="flex gap-4">
                                                <div className="mt-2 ml-2  w-40 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                    onClick={(e) => addCoverage(e, index)}>
                                                    <PlusCircleIcon className="w-5 h-5" />add coverage
                                                </div>
                                                <div className="mt-2 ml-2  w-40 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                    onClick={(e) => removeCoverage(e, index)}>
                                                    <MinusCircleIcon className="w-5 h-5" />remove coverage
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex  gap-4 items-end ">
                                                ...
                                                <fieldset className="border border-gray-400 p-4 rounded-md w-full mt-2 ">
                                                    ...
                                                    {
                                                        data?.potentialInsurancePolicy[index]?.coverage?.map((coverageItem: any, coverageIndex: number) => (
                                                            <>

                                                                <div key={coverageIndex} className=" w-full flex justify-between gap-4">
                                                                    <div className="flex-col w-full">
                                                                        <Select
                                                                            ...
                                                                            onChange={(e: any) => {
                                                                                const updatedPolicies = [...data.potentialInsurancePolicy];
                                                                                const updatedCoverage = [...updatedPolicies[index].coverage];
                                                                                updatedCoverage[coverageIndex] = {
                                                                                    coverage: e ? e.value : null,
                                                                                    note: ""
                                                                                };
                                                                                updatedPolicies[index].coverage = updatedCoverage;
                                                                                setData({
                                                                                    ...data,
                                                                                    potentialInsurancePolicy: updatedPolicies
                                                                                });
                                                                            ...
                                                                            value={
                                                                                data.potentialInsurancePolicy[index]?.coverage[coverageIndex]
                                                                                    ? coveragesOptions.find((el: { value: any }) => el.value === data.potentialInsurancePolicy[index]?.coverage[coverageIndex]?.coverage) || null
                                                                                    : null
                                                                            }
                                                                        />
                                                                        ...
                                                                        <TextArea
                                                                            ...
                                                                            onChange={(e: any) => {
                                                                                ...
                                                                                const updatedCoverage = [...updatedPolicies[index].coverage];
                                                                                updatedCoverage[coverageIndex] = {
                                                                                    coverage: data.potentialInsurancePolicy[index]?.coverage[coverageIndex]?.coverage,
                                                                                    note: e.target.value
                                                                                };
                                                                                updatedPolicies[index].coverage = updatedCoverage;
                                                                                setData({
                                                                            ...
                                                                            value={data.potentialInsurancePolicy[index]?.coverage[coverageIndex]?.note}
                                                                        />
                                                                        <span className="ml-2 text-sm italic text-gray-400"># add note if needed</span>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                {/* button add and remove row */}
                                                                <div className="flex gap-4">
                                                                    <div className="mt-2 ml-2  w-60 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                                        onClick={(e) => addInterestInsured(e, index)}>
                                                                        <PlusCircleIcon className="w-5 h-5" />add Interest Insured
                                                                    </div>
                                                                    <div className="mt-2 ml-2  w-60 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                                                        onClick={(e) => removeInterestInsured(e, index, coverageIndex)}>
                                                                        <MinusCircleIcon className="w-5 h-5" />remove Interest Insured
                                                                    </div>
                                                                </div>
                                                                {/* end button add and remove row */}
                                                                ...
                                                                {/* interest insured */}
                                                                {coverageItem.interestInsured && coverageItem.interestInsured.length > 0 && (
                                                                    <div className="mt-4">
                                                                        <fieldset className="border border-gray-400 p-4 rounded-md">
                                                                            <legend className="text-sm font-semibold px-2 text-gray-600">Interest Insured</legend>
                                                                            {coverageItem.interestInsured.map((interest: any, interestIndex: number) => (
                                                                                <div key={interestIndex} className="w-full flex justify-between gap-4">
                                                                                    <div className="flex-col w-full">
                                                                                        <Select
                                                                                            classNames={{
                                                                                                menuButton: () =>
                                                                                            ...
                                                                                            options={coverageInsuredDetailOptions}
                                                                                            onChange={(e: any) => {
                                                                                                const updatedPolicies = [...data.potentialInsurancePolicy];
                                                                                                const updatedCoverage = [...updatedPolicies[index].coverage];
                                                                                                const updatedInterest = [...updatedCoverage[coverageIndex].interestInsured];
                                                                                                updatedInterest[interestIndex] = {
                                                                                                    interestInsured: e ? e.value : null,
                                                                                                    note: ""
                                                                                                };
                                                                                                updatedCoverage[coverageIndex].interestInsured = updatedInterest;
                                                                                                updatedPolicies[index].coverage = updatedCoverage;
                                                                                                setData({
                                                                                                    ...data,
                                                                                                    potentialInsurancePolicy: updatedPolicies
                                                                                            ...
                                                                                            value={
                                                                                                interest.interestInsured
                                                                                                    ? coverageInsuredDetailOptions.find((el: { value: any }) => el.value === interest.interestInsured) || null
                                                                                                    : null
                                                                                            }
                                                                                        />
                                                                                        ...
                                                                                        <TextArea
                                                                                            ...
                                                                                            onChange={(e: any) => {
                                                                                                ...
                                                                                                const updatedInterest = [...updatedCoverage[coverageIndex].interestInsured];
                                                                                                updatedInterest[interestIndex] = {
                                                                                                    interestInsured: interest.interestInsured,
                                                                                                    note: e.target.value
                                                                                                };
                                                                                                updatedCoverage[coverageIndex].interestInsured = updatedInterest;
                                                                                                updatedPolicies[index].coverage = updatedCoverage;
                                                                                    ...
                                                                                    <div className="flex-col w-full">
                                                                                        <CurrencyInput
                                                                                            ...
                                                                                            onValueChange={
                                                                                                (value) => {
                                                                                                    ...
                                                                                                    updatedInterest[interestIndex] = {
                                                                                                        ...updatedInterest[interestIndex],
                                                                                                        ValueSubmitClaim: value ? parseFloat(value) : 0
                                                                                                    };
                                                                                                    updatedCoverage[coverageIndex].interestInsured = updatedInterest;
                                                                                                    updatedPolicies[index].coverage = updatedCoverage;
                                                                                                    setData({
                                                                                        ...
                                                                                        <TextArea
                                                                                            ...
                                                                                            onChange={(e: any) => {
                                                                                                ...
                                                                                                updatedInterest[interestIndex] = {
                                                                                                    ...updatedInterest[interestIndex],
                                                                                                    ValueSubmitClaim: interest.ValueSubmitClaim,
                                                                                                    noteValueSubmitClaim: e.target.value
                                                                                                };
                                                                                                updatedCoverage[coverageIndex].interestInsured = updatedInterest;
                                                                                                updatedPolicies[index].coverage = updatedCoverage;
                                                                                                setData({
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="mt-2 text-gray-500 text-sm flex gap-2 italic hover:text-red-400 cursor-pointer"
                                onClick={(e) => removeRow(e)}>
                                <MinusCircleIcon className="w-5 h-5" /> remove row
                            </div>
                        </div>
                ...
                body={
                    <>
                                        ...
                                        <div className=" text-lg font-medium italic mt-2">
                                            {dataId?.CLAIM_STATUS === 0 && (
                                                <div className=" flex gap-2 justify-center p-2 rounded-md text-blue-400 border-blue-400">
                                                    ...
                                                    <div className="flex items-center">Normal Progress</div>
                                                </div>
                                            )}
                                            {dataId?.CLAIM_STATUS === 1 && (
                                                <div className=" flex gap-2 justify-center p-2 rounded-md text-yellow-300 border-yellow-300">
                                                    <ExclamationTriangleIcon className="w-7 h-7 " />
                                                    <div className="flex items-center"> Potential Dispute</div>
                                                </div>
                                            )}
                                            {dataId?.CLAIM_STATUS === 2 && (
                                                <div className=" flex gap-2 justify-center p-2 rounded-md text-green-400 border-green-300">
                                                    <ExclamationTriangleIcon className="w-7 h-7" />
                                                    <div className="flex items-center">Potential Litigation</div>
                                                </div>
                                            )}
                                            {dataId?.CLAIM_STATUS === 3 && (
                                                <div className=" flex gap-2 justify-center p-2 rounded-md text-green-400 border-green-300">
                                                    <ScaleIcon className="w-7 h-7" />
                                                    <div className="flex items-center">Litigation</div>
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex">
                                            <div className="italic flex gap-2 justify-center p-2 rounded-md text-gray-500 hover:text-red-400 cursor-pointer"
                                                onClick={() => (setModal({ ...modal, stepper: true }))}>
                                                <FolderPlusIcon className="w-7 h-7" />
                                                <div className="flex items-center text-lg font-medium">Upload Documents</div>

                                            </div>
                                            <div className="italic flex gap-2 justify-center p-2 rounded-md text-gray-500 hover:text-red-400 cursor-pointer"
                                                onClick={() => (setModal({ ...modal, stepper: true }))}>
                                                <ClipboardDocumentListIcon className="w-7 h-7" />
                                                <div className="flex items-center text-lg font-medium">Claim Milestone</div>
                                    ...
                                    <div className="text-lg flex gap-2 text-center rounded-lg border border-gray-300 bg-slate-200">
                                        ...
                                        <div className=" w-full p-2 rounded-lg font-medium ">
                                            <div className="w-full rounded-lg h-10 mt-2">
                                                ...
                                                (  {dataId?.relation?.RELATION_ORGANIZATION_NAME})
                                            </div>
                                        </div>
                                                            ...
                                                            <>
                                                                <div className="mt-2 mb-4">
                                                                    <div className="bg-slate-200 border-gray-300 border rounded-md w-full p-4 h-24">
                                                                        <div className="text-lg text-center font-sans border-gray-300"> {el?.policy[0]?.POLICY_THE_INSURED} </div>
                                                                        <div className="text-lg text-center font-sans border-gray-300"> {el?.policy[0]?.POLICY_NUMBER} </div>
                                                                    </div>
                                                                    {
                                                                        dataId.coverage?.map((el: any) => {
                                                                            const currency = el?.insured?.[0]?.claim_coverage?.currency?.CURRENCY_SYMBOL
                                                                            const coverageName = el?.coverage?.POLICY_COVERAGE_NAME
                                                                            return (
                                                                                <>
                                                                                    <div className="px-4 mt-2 ">
                                                                                        ...
                                                                                        {el?.insured?.reduce((acc: any, curr: any) => {
                                                                                            ...
                                                                                            <div key={index} className={`rounded-md px-4 mt-3 p-2 h-10 ${index % 2 === 0 ? 'bg-slate-200' : ''}`}>
                                                                                                - {group.claim_coverage.interest_insured.INTEREST_INSURED_NAME}
                                                                                            </div>
                                                                                        ))}
                                                                                    </div>
                                                                                </>
                                                            ...
                                                            <>
                                                                <div className="mt-2 mb-4">
                                                                    <div className=" flex flex-row justify-center gap-2 bg-slate-200 border-gray-300 border rounded-md w-full p-4 h-24 text-2xl font-sans text-center items-center font-medium   ">
                                                                        ...
                                                                        {
                                                                            dataId.coverage?.map((el: any, index: number) => {
                                                                                if (!el?.insured?.length) return null; // Pastikan ada data

                                                                                // Group by POLICY_COVERAGE_DETAIL_ID
                                                                                const grouped = el.insured.reduce((acc: any, insured: any) => {
                                                                                    const policyId = insured.POLICY_COVERAGE_DETAIL_ID;
                                                                                    if (!acc[policyId] || acc[policyId].CLAIM_COVERAGE_INSURED_ID < insured.CLAIM_COVERAGE_INSURED_ID) {
                                                                                        acc[policyId] = insured; // Simpan insured dengan ID terbesar
                                                                                    }
                                                                                    return acc;
                                                                                }, {});

                                                                                // Ambil insured terakhir dari setiap POLICY_COVERAGE_DETAIL_ID dan jumlahkan VALUE_SUBMIT_COVERAGE
                                                                                const sumValue = Object.values(grouped).reduce((acc: number, insured: any) => {
                                                                                    return acc + (Number(insured?.VALUE_SUBMIT_COVERAGE) || 0);
                                                                                }, 0);

                                                                                // Ambil currency dari insured terakhir yang dipilih
                                                                                const firstGroupedItem = Object.values(grouped)[0];
                                                                                const currency = (firstGroupedItem as any)?.claim_coverage?.currency?.CURRENCY_SYMBOL || 'IDR';
                                                            ...
                                                            <>
                                                                <div className="mt-2 mb-4">
                                                                    <div className=" flex flex-row justify-center gap-2 bg-slate-200 border-gray-300 border rounded-md w-full p-4 h-24 text-2xl font-sans text-center items-center font-medium   ">
                                                                        ...
                                                                        {
                                                                            dataId.coverage?.map((el: any) => {
                                                                                const sumValue = el?.insured?.reduce((acc: any, el: any) => {
                                                                                    return acc + el?.VALUE_SUBMIT_COVERAGE
                                                                                }, 0)
                                                                                const currency = el?.insured?.[0]?.claim_coverage?.currency?.CURRENCY_SYMBOL
                                                                                return (
                                                                                    <>
                        ...
                        <ActionModal
                            ...
                            onClose={(e: any) => {
                                setModal({ ...modal, updateValue: false, edit: false })
                            }
                            }
                            data={valueRev}
                            ...
                            body={
                                <>
                                    ...
                                    <div className=" w-full p-2 rounded-lg ">
                                        {dataId?.coverage?.[0]?.coverage?.POLICY_COVERAGE_NAME}
                                    </div>
                                    <div className=" w-full p-2 rounded-lg ">
                                        {insuredId?.claim_coverage?.interest_insured?.INTEREST_INSURED_NAME}
                                    </div>
                                    <div className="relative">
                                        <div className="mb-2">
                                            ...
                                            <CurrencyInput
                                                ...
                                                onValueChange={
                                                    (value) => {
                                                        setValueRev((prev: any) => ({
                                                            ...prev,
                                                            VALUE_SUBMIT_COVERAGE: value ? parseFloat(value) : 0
                                                        }));
                                                    }
                                                }
                                                value={
                            ...
                            colDefs={[
                                ...
                                },
                                {
                                    ...
                                    valueGetter: (params: any) => {
                                        return params.data?.relation?.RELATION_ORGANIZATION_NAME || "-";
                                    }
                                    ,
                                },
