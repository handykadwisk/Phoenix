			...
			<div className="w-full mx-auto p-4">
              ...
              ${index === currentStep
									...
									{index < milestones?.length - 1 && (
										<div
											className={`flex-1 h-1 w-full ${index < currentStep
												? "bg-green-500"
												: "bg-gray-300"
												}`}
										/>
				...
				body={
					<>
						...
						<div className="h-auto p-4 ">
							<fieldset className="lg:flex lg:flex-row lg:gap-2 border border-gray-400 px-4 rounded-md lg:w-full mt-2 ">
								<legend className="text-lg font-semibold px-2 text-gray-600 mb-2">
									Claim Info
								</legend>
								<div className=" lg:w-full ">
									<div className="lg:flex lg:gap-2 lg:justify-between p-2">
									...
									{/* header */}
									<div className="lg:text-lg lg:flex gap-2 text-center rounded-lg border border-gray-300 bg-slate-200">
									...
									<fieldset className="mt-1 p-4 justify-end border border-gray-400 rounded-md mb-6">
										...
										<div className="lg:flex sm:hidden md:hidden  text-md bg-slate-200 border-gray-300 border flex gap-2 w-full text-center rounded-md font-semibold">
										...
										<div className="lg:px-3 lg:flex lg:space-x-2 lg:w-full mt-2">

											{/* Potential policy */}
											<div className="rounded-md p-2 h-15 w-full">
												<div className=" rounded-md p-2 h-15 sm:w-full lg:hidden">
													Potential Policy
												</div>
												{dataId.policy?.map((el: any) => (
													<div key={el.policy?.[0]?.POLICY_NUMBER} className="mt-2 mb-4">
														<div className="bg-slate-200 border-gray-300 border rounded-md w-full p-4 h-24">
															<div className="text-lg text-center font-sans border-gray-300">
																{el?.policy?.[0]?.POLICY_THE_INSURED}
															</div>
															<div className="text-lg text-center font-sans border-gray-300">
																{el?.policy?.[0]?.POLICY_NUMBER}
															</div>
														...
														{dataId.coverage?.map((coverage: any) => {
															const currency =
																coverage?.insured?.[0]?.claim_coverage?.currency
																	?.CURRENCY_SYMBOL;
															const coverageName =
																coverage?.coverage?.POLICY_COVERAGE_NAME;

															return (
																<div key={coverageName} className="px-4 mt-2  mb-4">
																	...
																	{coverage?.insured
																		...
																		.map((group: any, index: number) => (
																			<div
																				key={index}
																				className={`rounded-md px-4 mt-3 p-2 h-10 ${index % 2 === 0
																					? 'border border-gray-300 bg-slate-200'
																					: 'border border-gray-300'
																					}`}
																			>
																				-{' '}
																				{
																					group.claim_coverage
																						.interest_insured
																						.INTEREST_INSURED_NAME
																				}
																			</div>
																...
																<div className=" flex flex-row justify-center gap-2 bg-slate-200 border-gray-300 border rounded-md w-full p-4 h-24 text-2xl font-sans text-center items-center font-medium   ">
																	...
																	{dataId.coverage?.map((el: any, index: number) => {
																		...
																		const sumValue = Object.values(grouped).reduce((acc: number, insured: any) => {
																			return (
																				acc + (Number(insured?.VALUE_SUBMIT_COVERAGE) || 0)
																			);
																		}, 0
																		);
																		...
																		return (
																			<div key={index} className="w-full flex justify-center items-center gap-4">
																				{/* <div className="border">
																					{currency}
																				</div> */}
																				<div className="flex gap-2">
																					{CurrencyFormat(sumValue)}
																					<ExclamationCircleIcon className="w-6 h-8 text-gray-500 text-sm flex italic hover:text-red-400 cursor-pointer"
																						onClick={() => {
																							setModal(
																								{
																									...modal,
																			...
																			<div className="px-4 mt-2 text-center ">
																				...
																				<div className="">
																					{el?.insured?.reduce((acc: any, curr: any) => {
																						...
																						const value = +(latestValue?.VALUE_SUBMIT_COVERAGE);

																						return (
																							<>
																								<div key={index}
																									className={`rounded-md flex justify-end px-4 mt-3 p-2 max-h-10 ${index %
																										2 === 0 ? "border border-gray-300 bg-slate-200" : "border border-gray-300"}`}>
																									<div className="flex gap-2">
																										{CurrencyFormat(value)}
																										<CogIcon className="w-5 h-5 text-gray-500  hover:text-red-400 cursor-pointer"
																											onClick={(e) => {
																	...
																	{dataId.coverage?.map((el: any) => {
																		...
																		return (
																			<>
																				...
																				<div className="">
																					{
																						sumValue && el?.insured.some((item: any) => item.CREATED_BY === 2) ? (
																							CurrencyFormat(sumValue)
																						) : (
																							"no data yet"
																						)
																			...
																			// <PlusCircleIcon className="w-7 text-gray-500 hover:text-red-400 cursor-pointer" />
																		);
																	}
																	)}
																	{/* </div> */}
																						...
																						return (
																							<div key={index}
																								className={`rounded-md flex justify-end px-4 mt-3 p-2 max-h-10 ${index %
																									2 === 0 ? "border border-gray-300 bg-slate-200" : "border border-gray-300"}`}>
																								<div className="flex gap-2">
																									{group.values.some((item: any) => item.CREATED_BY === 2) ? (
																										<>
																											{CurrencyFormat(value)}
																											<CogIcon className="w-5 h-5 text-gray-500 hover:text-red-400 cursor-pointer"
																												onClick={(e) => {
							...
							{dataId.coverage?.map((coverage: any, index: number) => (
								<div key={index} className="mb-4">
									...
									{coverage.insured
										?.sort(
											(a: any, b: any) =>
												a.claim_coverage.POLICY_COVERAGE_DETAIL_ID -
												b.claim_coverage.POLICY_COVERAGE_DETAIL_ID
										)
										?.reduce((acc: any, curr: any) => {
											const existing = acc.find(
												(item: any) =>
										...
										?.map((group: any, groupIndex: number) => (
											<div key={groupIndex} className="mb-4">
												...
												{group.values.map(
													(insured: any, insuredIndex: number) => (
														<div
															key={insuredIndex}
															className={`p-2 border-b border-gray-300 ${insuredIndex % 2 === 0 ? "bg-gray-200" : ""
																}`}
														>
															<div className="flex justify-between">
