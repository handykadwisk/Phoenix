...
// import {} from 're'
export default function UserManagement({ auth, type, role, company, employee, relation, jobpost, divCompany
}: any) {
    ...
    const typeJson = type;
    const relationJson = relation;
    const jobpostJson = jobpost;
    const divCompanyJson = divCompany;
    ...
    //role for select 
    const roleFor = roleJson?.map((role: Role) => {
        return {
            value: role.id,
    ...
    //relation data
    const relationSelect = relationJson.map((el: any) => {
        return {
    ...
    //company
    const companySelect = companyJson.map((el: any) => {
        // console.log(el);
        return {
            value: el.COMPANY_ID,
    ...
    //division
    const employeeDiv = employeeJson.find((el: any) => dataInput.employee_id === el.EMPLOYEE_ID)?.division;
    const filterEmployeeDiv = divCompanyJson.filter((el: any) => el.COMPANY_ID === dataInput.company_id);
    const filterEmployeeDivEdit = divCompanyJson.filter((el: any) => el.COMPANY_ID === dataInputEdit.company_id);
    useEffect(() => {
        if (employeeDiv) {
            setDataInput((prevState) => ({
                ...prevState,
    ...
    //employee data
    const filterEmployee = employeeJson.filter((el: any) => el.COMPANY_ID === dataInput.company_id && el.EMPLOYEE_IS_DELETED === 0)
    const filterEmployeeEdit = employeeJson.filter((el: any) => el.COMPANY_ID === dataInputEdit.company_id && el.EMPLOYEE_IS_DELETED === 0)
    ...
    //jobpost data
    const filteredJobPosts = jobpostJson.filter((el: any) => el.company_division_id === dataInput.company_division_id);
    const filteredJobPostsEdit = jobpostJson.filter((el: any) => el.company_division_id === dataInputEdit.company_division_id);
    ...
    const optionRelation = relationJson.map((el: any) => {
        return {
            value: el.RELATION_ORGANIZATION_ID,
            label: el.RELATION_ORGANIZATION_NAME
        }
                        ...
                        <div className="mb-2">
                            ...
                            <select
                                ...
                                onChange={(e) => setDataInput({
                                    ...dataInput, type: Number(e.target.value),
                                    role: [], company_id: 0, company_division_id: 0, jobpost: 0, employee_id: 0,
                                    individual_relations_id: 0
                                }
                                )}
                            >
                                ...
                                {
                                    typeJson?.map((mType: any, i: number) => {
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setDataInputEdit({
                                                        ...
                                                        role: null, company_id: 0, company_division_id: 0, jobpost: 0, employee_id: 0,
                                                        individual_relations_id: 0
                                                    });
                                                }}
                                            >
                                        ...
                                        <>
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <Select
                                                    ...
                                                    isClearable={true}
                                                    value={relationSelect.find((emp: { value: any }) => emp.value === dataInputEdit.individual_relations_id) || ""}
                                                    onChange={handleInputChangeEdit('individual_relations_id')}
                                                    primaryColor={"red"}
                                                />
                                            </div>
                                            {/* company */}
