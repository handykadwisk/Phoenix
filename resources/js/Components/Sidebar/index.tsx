...
const Sidebar = ({
    ...
    return (
      <li key={index}>
        {menu.menu_url ? (
          ...
          <SidebarLinkGroup
            activeCondition={pathname.includes(menu.menu_name.toLowerCase())}
            key={index}
          >
            {(handleClick, open) => (
              <Fragment>
  ...
  const organizeMenu = (menu: MenuItem[], parentId: number | null = null): MenuItem[] => {
    // console.log(menu, '<<<<<<<<menuuuuu');

    const filteredMenu = menu.filter((item: MenuItem) => item.menu_parent_id === parentId);
